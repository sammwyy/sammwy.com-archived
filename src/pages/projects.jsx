import { Box, Heading, Image, Input, SimpleGrid, Tag, TagLabel, Text } from "@chakra-ui/react";
...
function ProjectDeck({ projects, categories }) {
    ...
    return (
        <Box>
            <Box>
                <Box>
                    <Input placeholder="Search anything" value={searchTerm} onChange={(e) => {
                        setSearchTerm(e.target.value);
                    }} />
                </Box>
                <Box marginTop={"10px"}>
                    <CategoryButton data={{label: "All", projects: projects.length, id: "*"}} />
                    {
                        categories.map((item, index) => <CategoryButton data={item} key={index} />)
                    }
                </Box>
            </Box>
...
export default function Projects() {
    ...
    return (
        <>
            {
                projects && categories && <ProjectDeck projects={projects} categories={categories} />
            }
        </>
    )
}